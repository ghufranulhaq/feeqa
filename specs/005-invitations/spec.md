...
2. **E-commerce integration.** When a store connects its e-commerce platform and an order is marked fulfilled, an invitation is queued automatically (service review, plus product reviews for the purchased SKUs if the plan includes them).
...
### Scheduling & sending
- **FR-005-08** Configurable per Business: send delay (0–30 days after the trigger, default 7), send window (local hours), reminder (0 or 1, sent 3–7 days after an unopened invitation), sender name, reply-to, and template per locale.
...
- Building every e-commerce connector in Phase 1. The launch connectors are chosen in the plan based on the launch vertical.
...
- **Q1:** Which e-commerce/booking integrations are needed for the launch vertical (e.g., travel booking engines, GDS, Shopify)?
- **Q2:** Default send delay per category (e.g., airlines: 1 day after the flight; hotels: 1 day after checkout). Should this be configured per category?
