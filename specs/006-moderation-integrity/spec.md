...
- **FR-006-12** Staff actions: publish, remove, redact (replace a span with `[removed]`), request verification, mark as not genuine, block account, restrict business feature, apply or lift a Consumer Warning, merge duplicates, freeze reviews. Every action requires a reason code and writes an audit event.
...
- [ ] The Transparency Center pages exist, and a generated quarterly report reconciles with raw events.
